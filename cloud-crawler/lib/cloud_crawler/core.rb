require 'thread'
require 'robotex'
require 'cloud_crawler/tentacle'
...
module CloudCrawler
  ...
  class Core
    ...
    def run
      ...
      #TODO   redis queue backed with bf to avoid repeats
      #  tentacle then polls redis?
      #  design carefully, implement, and test memory
      
      # for now, simply restrict size of link queue
      ...
      loop do
        ...
        links.each do |link|
          puts  [link, page.url.dup, page.depth + 1].join("\t")
          link_queue << [link, page.url.dup, page.depth + 1]
        end
        @pages.touch_urls links
